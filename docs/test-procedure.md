...
**This document covers Windows native**, which the manual keeps in Appendix D.
For the Docker versions, the install steps are in section 2 of the manual
(Appendix A for Linux), and 2.8 there shows how to exercise the whole path with
a recording. Stages 3 and 4 below apply to all three: the meeting-side checks do
not depend on how you run it.
